...
- **Install Anthropic CLI (`ant`):** `go install github.com/anthropics/anthropic-cli/cmd/ant@latest`
  - If Go is missing on Windows, install with `winget install --id GoLang.Go --exact --accept-package-agreements --accept-source-agreements`.
  - In an already-open Git Bash/MSYS shell after winget install, PATH may not refresh; use `'/c/Program Files/Go/bin/go.exe' install github.com/anthropics/anthropic-cli/cmd/ant@latest` and verify with `'/c/Users/<user>/go/bin/ant.exe' --version`, or open a new terminal.
  - Ensure `%USERPROFILE%\go\bin` (shown by `go env GOPATH`) is on PATH so `ant` works by name.
