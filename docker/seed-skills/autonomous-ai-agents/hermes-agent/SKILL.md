...
### Community Web UIs / Dashboards

When user asks to set up community Hermes web frontends, first verify repo health with `git ls-remote`, then clone into one parent directory and read each README before running install scripts. For Docker/Dokploy deployment patterns, route/security defaults, and Compose templates, see `references/docker-dokploy-community-ui.md`. Common repos and durable setup notes:

- `nesquena/hermes-webui`: README says native Windows bootstrap is not supported; use Linux/macOS/WSL2 or Docker. Do not try to force `python3 bootstrap.py` on native Windows unless upstream support changed.
- `outsourc-e/hermes-workspace`: Node 22+ app. Install with `pnpm install`, create `.env` from `.env.example`, set `HERMES_API_URL=http://127.0.0.1:8642` and `HERMES_DASHBOARD_URL=http://127.0.0.1:9119` when attaching to local Hermes services. On native Windows, `npm run dev` may fail because the script uses POSIX env assignment (`NODE_OPTIONS="..." vite dev`). Use direct Vite instead: `npx vite --host 127.0.0.1 --port 3000`, or patch the script to use `cross-env` if making a durable project change.
- `Kori-x/hermes-dashboard`: `npm install`, then `bash install.sh` installs the Hermes plugin and builds the dashboard. Run with `npm run dev`; default Vite port is `5173`.
- `Cranot/super-hermes`: installs skills/prisms via `bash install.sh` or `powershell -ExecutionPolicy Bypass -File install.ps1`. After install, skills are available as `/prism-scan`, `/prism-full`, `/prism-3way`, `/prism-discover`, `/prism-reflect`.
- Vite port collision: many frontends default to `5173`; if running Workspace and Dashboard together, move one with `--port 3000`.
- For enhanced Workspace panes, run Hermes API gateway (`hermes gateway run`) and Hermes dashboard service as expected by the UI; verify endpoints with `curl http://127.0.0.1:8642/health` and `curl http://127.0.0.1:9119/api/status` when available.
