...
403 Forbidden
401 Unauthorized
curl: (6) Could not resolve host: <production-domain>
```

**Diagnosis:**
1. Check if the workflow needs special permissions (token scopes).
2. Check if secrets are configured (missing `GITHUB_TOKEN` or custom secrets): `gh secret list --repo OWNER/REPO`.
3. For scheduled API-trigger workflows, separate domain, secret, and app problems:
   - DNS failure (`curl: (6) Could not resolve host`) means configured production base URL/domain is dead or not published. Confirm with `nslookup <host>` and switch workflow to a repo variable like `PROD_BASE_URL` instead of hardcoding a brittle domain.
   - `401 Unauthorized` from a live endpoint means the GitHub Actions secret likely differs from the deployed app secret. Do not print secrets; verify existence only, then have user sync GitHub secret and deployment env.
   - If no auth header reaches the app, inspect shell quoting in workflow YAML and avoid malformed `${{ secrets.X }}` syntax.
4. Check workflow logs with `gh run view <RUN_ID> --log-failed`; GitHub masks secret values as `***`, so diagnose by status code and endpoint, not by trying to reveal values.

**Common fixes:**
- Add `permissions:` block to workflow YAML.
- Verify secrets exist: `gh secret list` or check repo settings.
- Put production API base URL in repo variable, e.g. `gh variable set PROD_BASE_URL --repo OWNER/REPO --body https://app.example.com`.
- Use `curl --fail --show-error --silent --location --connect-timeout 20 --max-time 120 --retry 2` so future failures show clear DNS/401/5xx causes.
